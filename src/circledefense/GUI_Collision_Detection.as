/** 
 * File:    GUI_Collision_Detection
 * 
 * Original code from: Troy Gilbert
 ...
 * Description: 
 * 
 * This file contains code which determines if two graphic objects overlap.
 * It uses the "alpha" map of each object in a Graphics sense
 * 
 */ 
package circledefense
{
	...
	public class GUI_Collision_Detection
	{
		 ...
		 */
		public static function isColliding(target1:DisplayObject, target2:DisplayObject, pixelPrecise:Boolean = true, tolerance:Number = 0):Boolean
		{
			if (target1.parent != target2.parent)
			{
				trace("error: both objects must be children of the same gui container!");
				return false;
			}
			
			var collisionRect:Rectangle = getCollisionRect(target1, target2, target1.parent, pixelPrecise, tolerance);
